    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess($this->__('Stores was saved successfully'));
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess($this->__('Stores was deleted successfully'));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massDeleteAction()
    {
        ...
        if (is_array($news) && sizeof($news) > 0) {
            try {
                ...
                $this->_getSession()->addSuccess($this->__('Total of %d stores have been deleted', sizeof($news)));
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
        } else {
            $this->_getSession()->addError($this->__('Please select stores'));
        }
        $this->_redirect('*/*');
    }
